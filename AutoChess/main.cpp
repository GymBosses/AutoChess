...
const int scrX = 3000; //Size (меняет под свой экран, нужно тестировать)
const int scrY = 2000;
		...
		while (window.pollEvent(event))
		{
			...
			if (event.type == sf::Event::MouseButtonReleased && !battle) //отпускание кнопки мыши
			{
				if (event.key.code == sf::Mouse::Left)// не правая
				{
					...
					if (levelup.sprite.getGlobalBounds().contains(pos.x, pos.y)) //Повышение уровня магазина
					{
						if (player.up_level())
						{
